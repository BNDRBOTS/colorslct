...
(function () {
  ...
  /* ================= State ================= */
  var currentPalette = Object.assign({}, E.PRESETS.defaultPro);
  var historyStack = [];
  var compareSwatches = [];
  var wcagMode = false;
  ...
  function persistState() {
    Store.set("cslct-state", { palette: currentPalette, history: historyStack, wcagMode: wcagMode });
  }
  ...
  function updatePreview() {
    ...
    $("previewContainer").innerHTML =
      ...
      "</div>" +
      '<div class="preview-node" style="color:' + p.textPrimary + ';">BNDR node</div>' +
      "</div></div>" +
      ...
      '<button class="preview-btn" type="button" tabindex="-1" style="background:' + p.accent1 + "; color:" + accentText + ';">Accent</button>' +
      "</div>" +
      '<div class="preview-node" style="background:rgba(' + cRgb + ',0.8); color:' + p.bgLight + ';">BNDR node</div>' +
      "</div></div>";
  }
  ...
  function updateContrast() {
    var p = currentPalette;
    var pairs = [
      { label: "Text on light bg", fg: p.textPrimary, bg: p.bgLight },
      { label: "Primary on light bg", fg: p.primary, bg: p.bgLight },
      { label: "Accent on light bg", fg: p.accent1, bg: p.bgLight },
      { label: "Light bg text on dark bg", fg: p.bgLight, bg: p.bgDark },
      { label: "Primary on dark bg", fg: p.primary, bg: p.bgDark },
      { label: "Accent on dark bg", fg: p.accent1, bg: p.bgDark },
    ];
    $("contrastList").innerHTML = pairs.map(function (pair) {
      var ratio = E.contrast(pair.fg, pair.bg);
      var level = E.wcagLevel(ratio);
      var cls = level === "AAA" ? "aaa" : level === "AA" ? "aa" : "fail";
      return (
  ...
  /* ================= Core action buttons ================= */
  $("randomBtn").addEventListener("click", function () { applyNewPalette(randomOklchPalette()); });
  ...
  $("loadBNDR").addEventListener("click", function () { applyNewPalette(E.PRESETS.bndrColors); });
  $("resetPro").addEventListener("click", function () { applyNewPalette(E.PRESETS.defaultPro); });

  /* ================= Share link ================= */
  $("shareBtn").addEventListener("click", function () {
    var base = window.location.href.split("#")[0];
    var url = base + "#p=" + E.encodePalette(currentPalette);
    UI.copyText(url, function () { showToast("Share link copied — anyone who opens it sees this palette."); });
  });
  ...
  function renderLibrary() {
    ...
    if (!savedPalettes.length) {
      list.innerHTML = '<div class="library-empty">No palettes saved yet.<br>Press <strong>Save</strong> and your current colors will be kept right here.</div>';
      return;
    }
  ...
  $("generateAI").addEventListener("click", function () {
    ...
    fetch("https://api.deepseek.com/v1/chat/completions", {
      ...
      .catch(function (e2) {
        var msg = e2 && e2.name === "AbortError" ? "Request timed out — please try again." : "AI error: " + (e2 && e2.message ? e2.message : "unknown");
  ...
  $("buyStripe").addEventListener("click", function () {
    ...
    else { showToast("Stripe checkout isn\u2019t connected yet — the seller still needs to add a payment link."); }
  });
  ...
  /* ================= Guided tour ================= */
  var TOUR_STEPS = [
    { target: null, title: "Welcome to ColorSLCT!", body: "This little studio helps you pick five colors that look great together — and are easy for everyone to read. Let\u2019s take a quick walk. It takes about a minute." },
    ...
    { target: "actionBar", title: "The magic buttons", body: "\u201CRandom\u201D invents a fresh palette for you. \u201CHarmony\u201D gently spins your colors to a matching new mood. \u201CBack\u201D undoes. Try them as much as you like — nothing breaks." },
    ...
    { target: "contrastPanel", title: "The readability report", body: "Green dots mean easy to read. Red means squint city. Turn on \u201CWCAG Auto\u201D and the studio fixes hard-to-read colors for you, automatically." },
    { target: "exportBtn", title: "Take your colors home", body: "When you\u2019re happy, press \u201CExport\u201D to copy or download your colors as ready-to-use code. \u201CSave\u201D keeps palettes in your library; \u201CShare\u201D makes a link for friends." },
  ...
  if (!Store.get("cslct-tour-done", false) && !/redeem/.test(window.location.hash)) {
    ...
    if (Store.consent()) { startWhenReady(); }
    else { document.addEventListener("cslct:consent", startWhenReady, { once: true }); }
  }
