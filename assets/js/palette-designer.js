/* ColorSLCT Palette Designer v3 — a generative OKLCH engine, not a preset picker.
   ...
   3. Draws structure decisions (background tint direction, dark-surface hue anchor,
      which accent carries the volume, lightness split) from the seeded RNG, giving
      each palette its own construction — not one recipe with new numbers.
...
(function (root, factory) {
  ...
  function designPalette(opts) {
    ...
    var tintMode = Math.floor(rng() * 4); /* 0 base tint · 1 accent tint · 2 near-neutral · 3 complement whisper */
    var darkAnchor = rng() < 0.45 ? baseHue : (rng() < 0.7 ? accentHue : wrapHue(baseHue + 180));
    ...
    var bgHue = tintMode === 0 ? baseHue
      ...
      : baseHue;
    var bgL = 0.925 + rng() * 0.045;
    var bgC = tintMode === 2 ? 0.002 + rng() * 0.006 : pick(rng, M.bgTintFrac) * maxChroma(bgL, bgHue);
    var bgLight = toHex(bgL, bgC, wrapHue(bgHue + (rng() - 0.5) * 18));

    var darkHue = wrapHue(darkAnchor + (rng() - 0.5) * 24);
    var bgDark = toHex(pick(rng, M.darkL), pick(rng, M.darkChroma), darkHue);
    ...
    if (!seeded) {
      var sig = mood + "|" + Math.round(baseHue / 15) + "|" + Math.round(accent.H / 15) + "|" + tintMode + "|" + (swapLoudness ? 1 : 0);
