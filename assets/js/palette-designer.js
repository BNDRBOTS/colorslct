...
(function (root, factory) {
  ...
  function buildPalette(opts) {
    ...
    var darkCands = [wrapHue(darkAnchor + (rng() - 0.5) * 24)];
    if (M.warm) {
      darkCands.push(wrapHue(15 + rng() * 85), wrapHue(15 + rng() * 85));
    } else {
      darkCands.push(
        wrapHue(darkAnchor + 120 + (rng() - 0.5) * 40),
        wrapHue(darkAnchor - 120 + (rng() - 0.5) * 40),
    ...
    if (isPitch) {
      depth = "pitch";
      dL = 0.10 + rng() * 0.048;
      dC = Math.min(0.018, maxChroma(dL, darkHue)) * rng();
    } else {
    ...
    if (isInk) {
      ...
      var textCands = [
        wrapHue(baseHue + 180), wrapHue(accentHue + 180), darkHue,
        wrapHue(15 + rng() * 80), rng() * 360, rng() * 360
      ];
