...
(function (root, factory) {
  ...
  /* ---------- hue vocabularies (v3.6) ----------
     "Different and readable" is not enough: a dark tint also has to BELONG
     to its mood. A brown dark under Electric is a wrong answer even if it
     contrasts perfectly. Each mood declares the hue arcs its tinted darks
     and tinted text may live in; arcs are [start, end] with end allowed
     past 360 to wrap. Pitch/ink poles are hueless, so they are exempt. */
  function inArcs(h, arcs) {
    h = wrapHue(h);
    for (var i = 0; i < arcs.length; i++) {
      var s = arcs[i][0];
      var hh = h < s ? h + 360 : h;
      if (hh <= arcs[i][1]) return true;
    }
    return false;
  }
  function randInArcs(rng, arcs) {
    /* sample 6° inside each edge — hex round-trips drift hue slightly, and a
       tint flush against the vocabulary boundary reads as the neighbor hue */
    var margin = 6, total = 0, i, spans = [];
    for (i = 0; i < arcs.length; i++) {
      var s = arcs[i][1] - arcs[i][0];
      spans.push(s >= 355 ? s : Math.max(1, s - margin * 2));
      total += spans[i];
    }
    var r = rng() * total;
    for (i = 0; i < arcs.length; i++) {
      if (r <= spans[i]) {
        var off = (arcs[i][1] - arcs[i][0]) >= 355 ? r : margin + r;
        return wrapHue(arcs[i][0] + off);
      }
      r -= spans[i];
    }
    return wrapHue(arcs[0][0]);
  }
  function snapToArcs(h, arcs) {
    if (inArcs(h, arcs)) return wrapHue(h);
    var best = wrapHue(arcs[0][0]), bestD = 1e9;
    for (var i = 0; i < arcs.length; i++) {
      var span = arcs[i][1] - arcs[i][0];
      var inset = Math.min(14, span * 0.25);
      var edges = [wrapHue(arcs[i][0] + inset), wrapHue(arcs[i][1] - inset)];
      for (var j = 0; j < 2; j++) {
        var d = hueDist(h, edges[j]);
        if (d < bestD) { bestD = d; best = edges[j]; }
      }
    }
    return best;
  }
  ...
  var MOODS = {
    vivid: {
      ...
      tintArcs: [[0, 360]], /* Loud is omnivorous */
    },
    calm: {
      ...
      tintArcs: [[130, 300]], /* forest, teal, slate, navy, indigo — recessive cools */
    },
    neon: {
      ...
      tintArcs: [[170, 350]], /* petrol, electric blue, indigo, violet, magenta, hot pink — never earth */
    },
    pastel: {
      ...
      softText: true,
      tintArcs: [[170, 380]], /* soft aqua, navy, plum, mauve, dusty rose */
    },
    earthy: {
      ...
      warm: true,
      tintArcs: [[10, 165]], /* oxblood, espresso, olive, moss, forest */
    },
    professional: {
      ...
      tintArcs: [[180, 290]], /* petrol, navy, indigo — boardroom darks */
    },
  };
  ...
  /* v3.5: per-token hue memories. Measurement showed dark/text hue variety
     existed on paper but not to the eye — weak tints at depth read as one
     near-black, and the tints that DO survive darkness skew red/green. These
     memories force consecutive seedless runs into different sectors for the
     dark surface and the text tint. Seeded runs never touch them. */
  var recentDarkHues = [], recentTextHues = [];
  var TOKEN_HUE_MEMORY = 8;
  /* The uniqueness re-roll builds several candidates per click but the user
     only sees the KEPT one — so the memory alone cannot guarantee that two
     kept-in-a-row tints differ. lastKept holds the tinted hue the user last
     saw per token; candidates within 50° of it are heavily penalized. */
  var lastKeptDarkHue = null, lastKeptTextHue = null;
  function pickSpread(rng, memory, candidates, avoidHue) {
    var best = candidates[0], bestScore = -1;
    for (var i = 0; i < candidates.length; i++) {
      var minD = 180;
      for (var j = 0; j < memory.length; j++) {
        var d = hueDist(candidates[i], memory[j]);
        if (d < minD) minD = d;
      }
      var score = (0.08 + (minD / 180) * 0.92) * (0.85 + rng() * 0.3);
      if (avoidHue !== null && avoidHue !== undefined && hueDist(candidates[i], avoidHue) < 50) score *= 0.02;
      if (score > bestScore) { bestScore = score; best = candidates[i]; }
    }
    memory.push(wrapHue(best));
    if (memory.length > TOKEN_HUE_MEMORY) memory.shift();
    return best;
  }
  ...
  function buildPalette(opts) {
    ...
    /* dark surface, v3.5: two honest poles and nothing mushy between.
       pitch — a true hueless near-black (L 0.10-0.15, C -> 0);
       tinted — a dark you can NAME. Tint legibility is hue-dependent: at
       equal chroma a dark red reads colored while a dark navy reads black,
       which is why v3.4 still felt like maroon/forest cycling. Tinted darks
       now sit in the legible band and ride 45-100% of the gamut ceiling,
       with the hue spread away from recent runs by the per-token memory. */
    var isPitch = rng() < (M.keepDeepAccents ? 0.34 : 0.28);
    var arcs = M.tintArcs;
    var darkCands = [snapToArcs(darkAnchor + (rng() - 0.5) * 24, arcs)];
    darkCands.push(
      snapToArcs(darkAnchor + 120 + (rng() - 0.5) * 40, arcs),
      snapToArcs(darkAnchor - 120 + (rng() - 0.5) * 40, arcs),
      randInArcs(rng, arcs), randInArcs(rng, arcs), randInArcs(rng, arcs)
    );
    var darkHue = (seeded || isPitch) ? darkCands[0] : pickSpread(rng, recentDarkHues, darkCands, lastKeptDarkHue);
    var dL, dC, depth;
    if (isPitch) {
      depth = "pitch";
      dL = 0.10 + rng() * 0.048;
      dC = 0; /* truly hueless — tiny chroma at L 0.10 quantizes to a red-black hex */
    } else {
      depth = rng() < 0.5 ? "pigment" : "charcoal";
      dL = M.keepDeepAccents ? 0.20 + rng() * 0.06 : 0.21 + rng() * 0.085;
      var tintFloor = depth === "pigment" ? 0.65 : 0.45;
      dC = (tintFloor + rng() * (1 - tintFloor)) * maxChroma(dL, darkHue);
    }
    dL = Math.min(0.295, dL);
    var bgDark = toHex(dL, dC, darkHue);

    /* text, v3.5: the same two honest poles — near-black ink, or a tint
       you can name (wine, forest, navy, indigo, espresso). Tinted text rides
       35-95% of the gamut ceiling at its lightness so the tint survives the
       7:1 fit, and its hue has its own anti-repeat memory. */
    var isInk = rng() < 0.32;
    var textHue, textL, textC;
    if (isInk) {
      textHue = wrapHue(baseHue + (rng() < 0.5 ? 180 : 0));
      textL = 0.14 + rng() * 0.08;
      textC = 0.004 + rng() * 0.008;
    } else {
      var textCands = [
        snapToArcs(baseHue + 180, arcs), snapToArcs(accentHue + 180, arcs), snapToArcs(darkHue, arcs),
        randInArcs(rng, arcs), randInArcs(rng, arcs), randInArcs(rng, arcs)
      ];
      textHue = seeded ? textCands[Math.floor(rng() * textCands.length)] : pickSpread(rng, recentTextHues, textCands, lastKeptTextHue);
      textL = 0.17 + rng() * 0.13;
      var tFloor = M.softText ? 0.35 : 0.55;
      textC = (tFloor + rng() * (0.95 - tFloor)) * maxChroma(textL, textHue);
    }
    var textFit = fitTowards(textL, Math.min(textC, maxChroma(textL, textHue)), textHue, bgLight, 7, -1);
    var textPrimary = textFit.hex;
    if (!isInk) {
      /* the 7:1 fit descends lightness and strands chroma below visibility on
         bright papers — the tint dies in transit. Re-ride the ceiling at the
         FINAL lightness, backing off only if 7:1 would break. */
      var tfrac = 0.6 + rng() * 0.35;
      /* absolute floor: a tinted run must stay VISIBLY tinted at its final
         depth — ride at least C 0.045 whenever the ceiling allows, else the
         full ceiling. A fraction of a tiny ceiling is how tints die. */
      var rideC = function (L) {
        var ceil = maxChroma(L, textHue);
        return Math.min(ceil, Math.max(tfrac * ceil, 0.045));
      };
      var rL = textFit.L, rC = rideC(rL);
      var rHex = toHex(rL, rC, textHue), rGuard = 0;
      while (E.contrast(rHex, bgLight) < 7 && rL > 0.06 && rGuard++ < 30) {
        rL -= 0.012;
        rC = rideC(rL);
        rHex = toHex(rL, rC, textHue);
      }
      if (E.contrast(rHex, bgLight) >= 7) textPrimary = rHex;
    }
    ...
    var effM = M;
    if (resolved === "clash") {
      effM = Object.assign({}, M, { chromaFrac: [Math.max(0.94, M.chromaFrac[0]), 1.0], floorOvershoot: 2.2 });
    ...
    var dfin = E.hexToOklch(bgDark);
    if (dfin[1] < 0.045 && !inArcs(dfin[2], arcs)) {
      /* hue is quantization noise at this depth, not a decision — go neutral */
      bgDark = toHex(dfin[0], 0, 0);
    }
  ...
  function designPalette(opts) {
    ...
    var keptDark = E.hexToOklch(best.palette.bgDark);
    if (keptDark[1] >= 0.035) lastKeptDarkHue = keptDark[2];
    var keptText = E.hexToOklch(best.palette.textPrimary);
    if (keptText[1] >= 0.035) lastKeptTextHue = keptText[2];
    return best;
  }
