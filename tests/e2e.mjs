...
/* ============ F. Studio core controls ============ */
{
  ...
  await page.goto(URLS.app);
  await sleep(400);
  check("default palette loads", (await hexOf(page, "primary")) === "#0066cc");
  check("back disabled initially", await page.$eval("#backBtn", (el) => el.disabled));

  await page.click("#loadBNDR");
  check("Load BNDR applies preset", (await hexOf(page, "primary")) === "#ccff00");
  await page.click("#resetPro");
  check("Professional resets preset", (await hexOf(page, "primary")) === "#0066cc");
...
/* ============ H. Share link round-trip ============ */
{
  ...
  await sleep(400);
  check("share shows toast", /link copied/i.test(await toastText(page)));
  ...
  await sleep(500);
  check("share link restores palette", (await page2.$eval("#hex-primary", (el) => el.value)) === "#ccff00");
