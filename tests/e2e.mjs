...
/* ============ A. Landing: consent, demo, pricing, links ============ */
{
  ...
  check("stripe button absent without a payment link", !(await page.isVisible("#pricingStripe")));
  check("bmac button absent without a product link", !(await page.isVisible("#pricingBmac")));
  await page.evaluate(() => { CSLCT_CONFIG.stripe.paymentLink = "https://buy.stripe.com/test_123"; window.CslctSyncCheckout(); });
  check("stripe button appears once its link is configured", await page.isVisible("#pricingStripe"));
  await page.evaluate(() => { CSLCT_CONFIG.stripe.paymentLink = ""; window.CslctSyncCheckout(); });
  check("stripe button retracts when the link is removed", !(await page.isVisible("#pricingStripe")));
...
/* ============ J. Gift code -> Pro unlock -> deactivate ============ */
{
  ...
  check("studio stripe buy absent without a payment link", !(await page.isVisible("#buyStripe")));
  check("studio bmac buy absent without a product link", !(await page.isVisible("#buyBmac")));
  await page.evaluate(() => { CSLCT_CONFIG.stripe.paymentLink = "https://buy.stripe.com/test_123"; window.CslctOpenUpgrade(); });
  await sleep(200);
  check("studio stripe buy appears once configured", await page.isVisible("#buyStripe"));
  await page.evaluate(() => { CSLCT_CONFIG.stripe.paymentLink = ""; window.CslctOpenUpgrade(); });
  await sleep(200);
  check("studio stripe buy retracts when unconfigured", !(await page.isVisible("#buyStripe")));
  ...
  {
    const formats = await page.$$eval(".format-tab", (ts) => ts.map((t) => t.dataset.format));
    for (const f of formats) {
      await page.click(`.format-tab[data-format="${f}"]`);
      await sleep(150);
      const out = await page.$eval("#exportOutput", (el) => el.textContent);
      check(`export tab ${f}: renders output`, out.trim().length > 20, out.slice(0, 40));
      check(`export tab ${f}: tab selected`, await page.$eval(`.format-tab[data-format="${f}"]`, (el) => el.getAttribute("aria-selected") === "true" || el.classList.contains("active")));
    }
  }
  await page.keyboard.press("Escape");
  /* pro survives reload */
...
/* ============ P. Palette designer panel ============ */
{
  ...
  check("harmony select has 10 options", (await page.$$eval("#harmonySelect option", (o) => o.length)) === 10);
  await page.selectOption("#harmonySelect", "clash");
  await page.click("#randomBtn");
  await sleep(300);
  check("clash harmony regenerates with collision label", (await page.$eval("#designerWhy", (el) => el.textContent)).includes("Clash"));
  {
    const hues = await page.evaluate(() => {
      const E = window.ColorEngine;
      return [E.hexToOklch(document.getElementById("hex-primary").value)[2], E.hexToOklch(document.getElementById("hex-accent1").value)[2]];
    });
    const d = Math.min(Math.abs(hues[0] - hues[1]), 360 - Math.abs(hues[0] - hues[1]));
    check("clash accents collide >= 85 degrees", d >= 85, d.toFixed(0) + "°");
  }
...
/* ============ Q. Full-control sweep — every button, chip, tab, field ============ */
{
  const { page, context, errors } = await newPage();
  await page.goto(URLS.app);
  await sleep(500);
  /* every mood chip: activates, regenerates, persists */
  for (const mood of ["calm", "neon", "pastel", "earthy", "professional", "vivid"]) {
    const prev = await hexOf(page, "primary");
    await page.click(`.mood-chip[data-mood="${mood}"]`);
    await sleep(300);
    check(`mood chip ${mood}: active + pressed`, await page.$eval(`.mood-chip[data-mood="${mood}"]`, (el) => el.classList.contains("active") && el.getAttribute("aria-pressed") === "true"));
    check(`mood chip ${mood}: regenerates`, (await hexOf(page, "primary")) !== prev);
    check(`mood chip ${mood}: persisted`, await page.evaluate((m) => JSON.parse(localStorage.getItem("cslct-state")).designer.mood === m, mood));
  }
  /* every harmony option: regenerates, reports, persists */
  const harmonies = await page.$$eval("#harmonySelect option", (os) => os.map((o) => o.value));
  for (const h of harmonies) {
    await page.selectOption("#harmonySelect", h);
    await sleep(300);
    check(`harmony ${h}: why-line present`, (await page.$eval("#designerWhy", (el) => el.textContent)).length > 10);
    check(`harmony ${h}: persisted`, await page.evaluate((v) => JSON.parse(localStorage.getItem("cslct-state")).designer.harmony === v, h));
  }
  /* every token row: copy button + hex input round-trip */
  for (const t of ["bgLight", "textPrimary", "primary", "accent1", "bgDark"]) {
    await page.click(`.copy-hex-btn[data-token="${t}"]`);
    await sleep(150);
    check(`token ${t}: copy button copies`, /copied/i.test(await toastText(page)));
    await page.fill(`#hex-${t}`, "#3355aa");
    await page.$eval(`#hex-${t}`, (el) => { el.dispatchEvent(new Event("input", { bubbles: true })); el.blur(); });
    await sleep(150);
    check(`token ${t}: hex input accepts a valid hex`, /^#[0-9a-f]{6}$/.test(await hexOf(page, t)));
  }
  /* every modal opens and closes from its own Close/Cancel button */
  const modals = [["#exportBtn", "exportModal"], ["#aiBtn", "aiModal"], ["#saveBtn", "saveModal"], ["#libraryBtn", "libraryModal"], ["#upgradeBtn", "upgradeModal"]];
  for (const [btn, modal] of modals) {
    await page.click(btn);
    await page.waitForSelector(`#${modal}.open`, { timeout: 3000 });
    check(`${modal}: opens from ${btn}`, true);
    await page.click(`#${modal} [data-close-modal]`);
    await sleep(250);
    check(`${modal}: closes from its own button`, !(await page.isVisible(`#${modal}.open`)));
  }
  /* tour walks to the end on Next alone */
  await page.click("#helpBtn");
  await page.waitForSelector("#tourCard", { timeout: 3000 });
  let steps = 0;
  while (steps++ < 20 && (await page.$("#tourOverlay.open"))) {
    /* DOM-level click: the overlay swaps cards between steps, and a stale
       button handle fails Playwright hit-testing after the swap */
    const clicked = await page.evaluate(() => { const b = document.getElementById("tourNext"); if (b) { b.click(); return true; } return false; });
    if (!clicked) break;
    await sleep(250);
  }
  check("tour: Next walks every step to the finish", !(await page.$("#tourOverlay.open")) && steps <= 9, `${steps} clicks`);
  /* theme toggle round-trip */
  const th1 = await page.getAttribute("html", "data-theme");
  await page.click("[data-theme-toggle]");
  await sleep(150);
  const th2 = await page.getAttribute("html", "data-theme");
  await page.click("[data-theme-toggle]");
  await sleep(150);
  check("theme toggle: flips and flips back", th1 !== th2 && (await page.getAttribute("html", "data-theme")) === th1, `${th1}->${th2}`);
  /* electric mood never ships earth-tone darks in the real UI */
  await page.click('.mood-chip[data-mood="neon"]');
  await sleep(250);
  let earth = 0;
  for (let i = 0; i < 10; i++) {
    ...
    const stray = await page.evaluate(() => {
      const E = window.ColorEngine;
      const [, C, H] = E.hexToOklch(document.getElementById("hex-bgDark").value);
      const h = ((H % 360) + 360) % 360;
      return C >= 0.035 && h >= 20 && h <= 160;
    });
    if (stray) earth++;
  }
  check("electric mood: zero earth-tone darks in 10 UI runs", earth === 0, String(earth));
  check("sweep: no console errors", errors.length === 0, errors.join(" | "));
  await context.close();
}
