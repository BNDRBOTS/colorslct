...
/* Neutral fixture palette used only by tests — the shipped engine has no canned palettes. */
const FIX = { bgLight: "#f4f4f6", textPrimary: "#1d1d1f", primary: "#0066cc", accent1: "#e0004d", bgDark: "#000000" };
...
check("fixture palette valid", E.isValidPalette(FIX));
check("engine ships no canned palettes", !("PRESETS" in E));
check("invalid palette rejected (missing token)", !E.isValidPalette({ bgLight: "#fff" }));
check("invalid palette rejected (bad hex)", !E.isValidPalette({ ...FIX, primary: "blue" }));
...
const enc = E.encodePalette(FIX);
eq("encodePalette format", enc, "f4f4f6-1d1d1f-0066cc-e0004d-000000");
check("decodePalette roundtrip", JSON.stringify(E.decodePalette(enc)) === JSON.stringify(E.cleanPalette(FIX)));
...
const D = require(path.join(here, "../assets/js/palette-designer.js"));
{
  ...
  /* Per-mood contrast policy: neon is dark-surface-first (accents >= 3.5 on
     bgDark; a fluorescent color cannot also hit 3.5 on near-white). Every
     other mood guarantees accents >= its light floor (3.5; 3.0 pastel) on
     bgLight and >= 3 on bgDark. */
  let sweepFails = 0;
  for (const h of Object.keys(D.HARMONIES)) for (const m of Object.keys(D.MOODS)) for (let s = 1; s <= 6; s++) {
    ...
    if (meta.contrastRef === "dark") {
      if (E.contrast(p.primary, p.bgDark) < 3.48) sweepFails++;
      if (E.contrast(p.accent1, p.bgDark) < 3.48) sweepFails++;
    } else {
      const lf = meta.accentFloor !== undefined ? meta.accentFloor : 3.5;
      if (E.contrast(p.primary, p.bgLight) < lf - 0.02) sweepFails++;
      if (E.contrast(p.accent1, p.bgLight) < lf - 0.02) sweepFails++;
      const df = meta.darkFloor !== undefined ? meta.darkFloor : 3;
      if (E.contrast(p.primary, p.bgDark) < df - 0.02) sweepFails++;
      if (E.contrast(p.accent1, p.bgDark) < df - 0.02) sweepFails++;
    }
    if (E.contrast(p.bgLight, p.bgDark) < 5.48) sweepFails++;
  }
  ...
  /* Backgrounds must vary — not one near-white paper + one muted near-black.
     Deterministic seeds so this cannot flake: over 60 vivid runs, a real share
     of light surfaces must carry visible tint across several hue families,
     and a real share of dark surfaces must carry visible pigment. */
  {
    let tintedPapers = 0, richDarks = 0, luminousPapers = 0;
    const paperSectors = new Set();
    const paperLs = [];
    for (let s = 1; s <= 60; s++) {
      const { palette: p } = D.designPalette({ seed: (s * 2654435761) % 4294967296, mood: "vivid" });
      const bl = E.hexToOklch(p.bgLight), bd = E.hexToOklch(p.bgDark);
      paperLs.push(bl[0]);
      if (bl[1] >= 0.03) { tintedPapers++; paperSectors.add(Math.floor(((bl[2] % 360) + 360) % 360 / 45)); }
      if (bl[0] <= 0.905 && bl[1] >= 0.8 * D.maxChroma(bl[0], bl[2])) luminousPapers++;
      if (bd[1] >= 0.045) richDarks++;
    }
    check("light surfaces get real tint beyond near-white (" + tintedPapers + "/60)", tintedPapers >= 8);
    check("tinted papers span several hue families (" + paperSectors.size + " sectors)", paperSectors.size >= 3);
    check("dark surfaces get real pigment beyond near-black (" + richDarks + "/60)", richDarks >= 6);
    check("paper lightness itself varies (spread >= 0.03)", Math.max(...paperLs) - Math.min(...paperLs) >= 0.03);
    check("luminous papers ride the gamut ceiling (" + luminousPapers + "/60)", luminousPapers >= 8);
    check("papers reach the luminous band (min L <= 0.905)", Math.min(...paperLs) <= 0.905);
    check("papers never fall below the light floor (min L >= 0.845)", Math.min(...paperLs) >= 0.845);
  }
  /* Moods must be visibly distinct — each button its own identity.
     Deterministic seeds; 30 runs per mood, comparing measured averages. */
  {
    const statsFor = (mood) => {
      const aL = [], aC = [], papers = {}; let warm = 0;
      for (let s = 1; s <= 30; s++) {
        const { palette: p, meta } = D.designPalette({ seed: s * 26544359 + 11, mood });
        const pr = E.hexToOklch(p.primary), ac = E.hexToOklch(p.accent1);
        aL.push((pr[0] + ac[0]) / 2); aC.push((pr[1] + ac[1]) / 2);
        papers[meta.paper] = (papers[meta.paper] || 0) + 1;
        if ((pr[2] >= 15 && pr[2] <= 100) || (ac[2] >= 15 && ac[2] <= 100)) warm++;
      }
      const avg = (a) => a.reduce((x, y) => x + y, 0) / a.length;
      return { aL: avg(aL), aC: avg(aC), papers, warm };
    };
    const vividS = statsFor("vivid"), calmS = statsFor("calm"), pastelS = statsFor("pastel"),
      earthyS = statsFor("earthy"), proS = statsFor("professional");
    check("pastel accents lighter than calm (" + pastelS.aL.toFixed(2) + " vs " + calmS.aL.toFixed(2) + ")", pastelS.aL > calmS.aL + 0.08);
    check("calm accents clearly muted vs vivid (" + calmS.aC.toFixed(3) + " vs " + vividS.aC.toFixed(3) + ")", calmS.aC < vividS.aC * 0.55);
    check("earthy stays warm most runs (" + earthyS.warm + "/30)", earthyS.warm >= 18);
    check("pro keeps near-white paper majority (" + (proS.papers.porcelain || 0) + "/30)", (proS.papers.porcelain || 0) >= 14);
    check("earthy accents deeper than pastel", earthyS.aL < pastelS.aL - 0.1);
    check("vivid accents louder than pro (" + vividS.aC.toFixed(3) + " vs " + proS.aC.toFixed(3) + ")", vividS.aC > proS.aC + 0.04);
  }
  ...
  const spin = D.spinPalette(FIX, { seed: 5, harmony: "triadic", mood: "vivid" });
  check("spinPalette changes primary", spin.palette.primary !== FIX.primary);
